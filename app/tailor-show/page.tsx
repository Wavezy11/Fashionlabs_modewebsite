...
// Herbruikbare Brandon Component
const BrandonSection = ({ isSecond = false }: { isSecond?: boolean }) => (
  <div className="bg-[#1a1a1a] px-6 py-8 text-center">
    <h2 className="text-white text-2xl font-bold mb-6">Brandon</h2>
    ...
    <p className="text-white text-sm leading-relaxed max-w-sm mx-auto">
      Deze collectie is een artistieke en rebelse verkiezing van{" "}
      {isSecond && <span className="font-bold">mentale gezondheid</span>}
      {!isSecond && "mentale gezondheid"} en de impact van sociale media op onze samenleving. Elk kledingstuk draagt de
      sporen van innerlijke strijd en chaos, maar ook van kracht, intelligentie en creativiteit. In plaats van de
      duistere kant te verbergen, wordt deze omarmd als een{" "}
      {isSecond && <span className="font-bold">essentieel onderdeel</span>}
      {!isSecond && "essentieel onderdeel"} van het mens-zijn.
    </p>
  </div>
)

export default function FashionShowMainPage() {
        ...
        <div className="scroll-container h-full pt-[0] overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] overscroll-none">
          ...
          <main className="bg-white relative">
            ...
            <div className="bg-white px-6 py-8 text-center relative z-10">
              <h1 className="text-2xl font-bold tracking-wide text-black">Fashion Show</h1>
            </div>
            ...
            {/* Unique Fashion Show Bullet Points Section */}
            <div className="bg-[#B8A5D1] px-6 py-6">
              <ul className="space-y-3 text-white text-sm max-w-sm mx-auto">
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">The White screenJeans it up</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">Fashion_Meets_Gambia</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">Excellent hair dressers</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">AI makes you're business</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">Stunning red</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">Laser Layers</span>
                </li>
                <li className="flex items-start leading-relaxed">
                  <span className="text-white mr-3 mt-1.5 text-xs">•</span>
                  <span className="font-bold">What's your statement?</span>
                </li>
              </ul>
            </div>
