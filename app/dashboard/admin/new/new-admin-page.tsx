...
export default function NewAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess("Admin gebruiker succesvol aangemaakt!")
        setFormData({
          username: "",
          email: "",
          password: "",
        ...
        <div className="scroll-container h-full pt-[0] overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] overscroll-none">
          ...
          <main className="bg-white relative">
            ...
            <div className="bg-white px-6 py-4">
              <Link href="/dashboard" className="text-[#9480AB] hover:text-[#7a6b8a] font-medium">
            ...
            <div className="flex flex-col items-center justify-center px-8 py-8">
              <h1 className="text-3xl font-bold mb-8">NIEUWE ADMIN</h1>

              {error && <div className="bg-red-100 text-red-800 p-3 rounded mb-4 text-sm w-full">{error}</div>}
              {success && <div className="bg-green-100 text-green-800 p-3 rounded mb-4 text-sm w-full">{success}</div>}
              ...
              <form onSubmit={handleSubmit} className="w-full space-y-4">
                ...
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {isSubmitting ? "BEZIG..." : "ADMIN AANMAKEN"}
                  </button>
                </div>
              </form>
            </div>
